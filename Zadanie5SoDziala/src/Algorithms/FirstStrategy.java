        ...
        while (!getTasks().isEmpty() || processorsHaveTasks()) {
            ...
            for (Processor processor : getProcessors()) {
                if (!processor.getTasks().isEmpty()) {
                    ...
                    // Iteruję od końca, bo będę usuwać elementy z listy i nie chcę mieć problemów z indeksami
                    for (int i = tasksOnProcessor.size() - 1; i >= 0; i--) {
                        Task task = tasksOnProcessor.get(i);
                        // Zmniejszam pozostały czas zadania o czas, który upłynął w tej iteracji symulacji
                        ...
                        if (task.getRemainingTime() <= 0) {
                            // Zmniejszam obciążenie procesora, bo zadanie zostało zakończone
           ...
           // System.out.println(timeElapsedThisCycle + " ms, iteracja " + iterationCounter + ", pozostałe zadania: " + getTasks().size() +  processorsHaveTasks());
